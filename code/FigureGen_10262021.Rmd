...
Bench_fpath <- "/v1.8b7_10102021/Navaroo_LFQBench/Report (1).tsv" #LFQ bench file path
...
#mTRAQ <- mTRAQ[which(mTRAQ$Ms1.Area>0),]  # must have non-zero Ms1 quant
mTRAQ$Label <- "mTRAQ"
...
#unlab <- unlab[which(unlab$Ms1.Area>0),]  # must have non-zero Ms1 quant
...
Calculate number of PGs intersected across 3 samples (plexDIA and LF-DIA):
```{r}

dat <- fread(fpath, select = c("Run","Protein.Group","Protein.Names","Genes", "Modified.Sequence", "Stripped.Sequence","Precursor.Id", "Precursor.Charge", "Lib.Q.Value","Lib.PG.Q.Value", "Translated.Q.Value", "Proteotypic","Precursor.Translated", "Ms1.Area", "Ms1.Translated", "Quantity.Quality", "Ms1.Profile.Corr", "Spectrum.Similarity", "Precursor.Quantity",
                               "Mass.Evidence", "CScore", "Fragment.Correlations"))


df <- dat
df <- df[which(df$Ms1.Area>0),]
df <- df[which(df$Lib.PG.Q.Value<0.01),]
df0 <- df[grepl("wJD804", df$Run),] #%>% filter(grepl("mTRAQ0", Modified.Sequence))
df0 <- df0 %>% mutate("channel_name" = ifelse(grepl("mTRAQ0", Modified.Sequence), "mTRAQ0",
                                            ifelse(grepl("mTRAQ4", Modified.Sequence), "mTRAQ4", "mTRAQ8")))
df0$prot_lab <- paste0(df0$Protein.Group, df0$channel_name)
df0 <- df0 %>% distinct(prot_lab, .keep_all=T)
df0 <- df0 %>% add_count(Protein.Group)
df0 <- df0 %>% filter(n==3)
nrow(df0)/3
...
##################
dat <- fread(LF_fpath, select = c("Run","Protein.Group","Protein.Names","Genes", "Modified.Sequence", "Stripped.Sequence","Precursor.Id", "Precursor.Charge", "Lib.Q.Value","Lib.PG.Q.Value", "Translated.Q.Value", "Proteotypic","Precursor.Translated", "Ms1.Area", "Ms1.Translated", "Quantity.Quality", "Ms1.Profile.Corr", "Spectrum.Similarity", "Precursor.Quantity",
                               "Mass.Evidence", "CScore", "Fragment.Correlations"))

df <- dat
df <- df[which(df$Ms1.Area>0),]
df <- df[which(df$Lib.PG.Q.Value<0.01),]
df0 <- df[grepl("wJD757|wJD760|wJD763", df$Run),] #%>% filter(grepl("mTRAQ0", Modified.Sequence))
df0$prot_lab <- paste0(df0$Protein.Group, df0$Run)
df0 <- df0 %>% distinct(prot_lab, .keep_all=T)
df0 <- df0 %>% add_count(Protein.Group)
df0 <- df0 %>% filter(n==3)
nrow(df0)/3
...
#umelt1 <- umelt[,c(3,4,6,8,10,11,13,14,15,18,21,22,23,24,25,27,28,29,30,31,32,35,38,39,40,41,42,44,45,46,48,49,56,58)]
...
#select 20 most differntial Go-terms
#uni_Go <- umelt %>% dplyr::arrange(desc(abs(log2(as.numeric(value))))) %>% distinct(Var1, .keep_all=T) %>% slice_head(n=40)
...
Supp. Figure 3: CV of plexDIA with same and different labels and quant acc.
```{r}

#### show out-of-set quant (LFQ-bench style plots)
dat <- fread(fpath, select = c("Run","Protein.Group","Protein.Names","Genes", "Modified.Sequence", "Stripped.Sequence","Precursor.Id", "Precursor.Charge", "Lib.Q.Value","Lib.PG.Q.Value", "Translated.Q.Value", "Proteotypic","Precursor.Translated", "Ms1.Area", "Ms1.Translated", "Quantity.Quality", "Ms1.Profile.Corr", "Spectrum.Similarity", "Precursor.Quantity",
                               "Mass.Evidence", "CScore", "Fragment.Correlations"))


df <- dat
df0 <- df[grepl("wJD803", df$Run),] %>% filter(grepl("mTRAQ0", Modified.Sequence))
df0 <- df0 %>% mutate("channel_name" = ifelse(grepl("mTRAQ0", Modified.Sequence), "mTRAQ0",
                                            ifelse(grepl("mTRAQ4", Modified.Sequence), "mTRAQ4", "mTRAQ8")))

df4 <- df[grepl("wJD804", df$Run),] %>% filter(grepl("mTRAQ4", Modified.Sequence))
df4 <- df4 %>% mutate("channel_name" = ifelse(grepl("mTRAQ0", Modified.Sequence), "mTRAQ0",
                                            ifelse(grepl("mTRAQ4", Modified.Sequence), "mTRAQ4", "mTRAQ8")))
...
protein.groups_n$d0_d4 <- as.numeric(protein.groups_n$wJD803mTRAQ0)/as.numeric(protein.groups_n$wJD804mTRAQ4)
protein.groups_n_omit <- protein.groups_n[!is.na(protein.groups_n$d0_d4), ]
med_human <- protein.groups_n_omit[grepl("H. sapiens", protein.groups_n_omit$species),]
med_human <- median(med_human$d0_d4, na.rm=T)
scalar <- 1/med_human
...
#dat_1hr <- dat
df <- dat_LF
df <- df[grepl("wJD757_re|wJD760_re", df$Run),]
df <- df %>% mutate("channel_name" = ifelse(grepl("wJD757_re", Run), "mTRAQ0",
...
protein.groups_n$d0_d4 <- as.numeric(protein.groups_n$wJD757_remTRAQ0)/as.numeric(protein.groups_n$wJD760_remTRAQ4)
protein.groups_n_omit_LF <- protein.groups_n[!is.na(protein.groups_n$d0_d4), ]
med_human <- protein.groups_n_omit_LF[grepl("H. sapiens", protein.groups_n_omit_LF$species),]
med_human <- median(med_human$d0_d4, na.rm=T)
scalar <- 1/med_human
...
protein.groups_n_omit <- protein.groups_n_omit %>% dplyr::select("wJD804mTRAQ4", "species", "d0_d4", "PGs")
colnames(protein.groups_n_omit) <- c("sample2_int", "species", "Prot_rat", "PGs")
protein.groups_n_omit$sample2_int <- as.numeric(protein.groups_n_omit$sample2_int)
...
df8 <- df[grepl("wJD804", df$Run),] %>% filter(grepl("mTRAQ8", Modified.Sequence))
df8 <- df8 %>% mutate("channel_name" = ifelse(grepl("mTRAQ0", Modified.Sequence), "mTRAQ0",
                                            ifelse(grepl("mTRAQ4", Modified.Sequence), "mTRAQ4", "mTRAQ8")))
...
protein.groups_n$d0_d4 <- as.numeric(protein.groups_n$wJD803mTRAQ0)/as.numeric(protein.groups_n$wJD804mTRAQ8)
protein.groups_n_omit <- protein.groups_n[!is.na(protein.groups_n$d0_d4), ]
med_human <- protein.groups_n_omit[grepl("H. sapiens", protein.groups_n_omit$species),]
med_human <- median(med_human$d0_d4, na.rm=T)
scalar <- 1/med_human
...
df <- dat_LF
df <- df[grepl("wJD757_re|wJD763_re", df$Run),]
df <- df %>% mutate("channel_name" = ifelse(grepl("wJD757_re", Run), "mTRAQ0",
                                            ifelse(grepl("wJD763_re", Run), "mTRAQ8", "mTRAQ4")))
df$Unlabelled.Precursor <- paste0(df$Modified.Sequence, df$Precursor.Charge)
df$Unlabelled.Precursor <- str_remove_all(df$Unlabelled.Precursor, "\\(mTRAQ0\\)")
...
protein.groups_n$d0_d4 <- as.numeric(protein.groups_n$wJD757_remTRAQ0)/as.numeric(protein.groups_n$wJD763_remTRAQ8)
protein.groups_n_omit_LF <- protein.groups_n[!is.na(protein.groups_n$d0_d4), ]
med_human <- protein.groups_n_omit_LF[grepl("H. sapiens", protein.groups_n_omit_LF$species),]
med_human <- median(med_human$d0_d4, na.rm=T)
scalar <- 1/med_human
...
protein.groups_n_omit <- protein.groups_n_omit %>% dplyr::select("wJD804mTRAQ8", "species", "d0_d4", "PGs")
colnames(protein.groups_n_omit) <- c("sample2_int", "species", "Prot_rat", "PGs")
protein.groups_n_omit$sample2_int <- as.numeric(protein.groups_n_omit$sample2_int)

protein.groups_n_omit_LF <- protein.groups_n_omit_LF %>% dplyr::select("wJD763_remTRAQ8", "species", "d0_d4", "PGs")
colnames(protein.groups_n_omit_LF) <- c("sample2_int", "species", "Prot_rat", "PGs")
protein.groups_n_omit_LF$sample2_int <- as.numeric(protein.groups_n_omit_LF$sample2_int)
...
df <- dat
df4 <- df[grepl("wJD803", df$Run),] %>% filter(grepl("mTRAQ4", Modified.Sequence))
df4 <- df4 %>% mutate("channel_name" = ifelse(grepl("mTRAQ0", Modified.Sequence), "mTRAQ0",
                                            ifelse(grepl("mTRAQ4", Modified.Sequence), "mTRAQ4", "mTRAQ8")))
...
protein.groups_n$d0_d4 <- as.numeric(protein.groups_n$wJD803mTRAQ4)/as.numeric(protein.groups_n$wJD804mTRAQ8)
protein.groups_n_omit <- protein.groups_n[!is.na(protein.groups_n$d0_d4), ]
med_human <- protein.groups_n_omit[grepl("H. sapiens", protein.groups_n_omit$species),]
med_human <- median(med_human$d0_d4, na.rm=T)
scalar <- 1/med_human
...
figure_fin <- annotate_figure(figure_fin, top = text_grob("plexDIA out-of-set quantiation\n(Replicate 1 / Replicate 2)\n",color = "black", face = "bold", size = 20))

ggsave("SFigure_MS1_outofset_LFQBench.pdf", figure_fin, width=11,height=9, dpi=500)
